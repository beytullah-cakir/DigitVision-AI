...
def predict():
    ...
    # Base64'den görüntüyü çöz
    image_data = data['image'].split(',')[1]
    image_bytes = base64.b64decode(image_data)
    image = Image.open(io.BytesIO(image_bytes)).convert('L') # Gri tonlama

    # --- GELİŞMİŞ ÖN İŞLEME (MNIST Formatına Uygun) ---
    # 1. Rakamın bulunduğu alanı (bounding box) bul
    ...
    # 3. Kenar oranını koruyarak kareleştir ve merkeze al
    ...
    # 4. 20x20 boyuta getir (MNIST standardı: Rakam 20x20 alana sığmalı)
    new_image = new_image.resize((20, 20), Image.Resampling.LANCZOS)
    
    # 5. 28x28'e tamamlamak için 4 piksel padding ekle
    final_image = Image.new('L', (28, 28), (0))
    final_image.paste(new_image, (4, 4))

    # Hazır veriyi diziye çevir ve normalize et
